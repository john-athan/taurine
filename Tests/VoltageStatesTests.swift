...
func runVoltageStatesTests() {
    ...
    Check.suite("voltage-states: what is not a frequency table") {
        // The plain voltage-states1 on this Mac. Descending, reciprocal, and
        // reading it as megahertz makes a loaded machine look idle.
        Check.isNil(VoltageStates.frequenciesMHz(in: table([
            (64250, 590), (46678, 640), (36653, 710), (25283, 880),
    ...
    Check.suite("voltage-states: scale is measured, not looked up by chip name") {
        // Apple has shipped these in hertz (M1 through M3, and this M4's GPU)
        // and in kilohertz (this M4's CPU). Both land in the same place.
    ...
    Check.suite("voltage-states: looking a table up by shape alone") {
        let states = VoltageStates(
            domains: [:],
            tables: [
                .init(index: 9, isSRAM: true, frequenciesMHz: [338, 618, 1578]),
                .init(index: 14, isSRAM: true, frequenciesMHz: [338, 618, 1578]),
                .init(index: 31, isSRAM: false, frequenciesMHz: [338, 618, 992]),
                .init(index: 28, isSRAM: false, frequenciesMHz: [801, 1602]),
                .init(index: 29, isSRAM: false, frequenciesMHz: [534, 801]),
            ])

        // Two sram candidates that agree are not an ambiguity, and the two
        // non-sram tables of the same shape do not get a vote while an sram
        // candidate exists. This is the M4 Pro's GPU exactly.
        Check.equal(states.frequenciesMHz(operatingStates: 3)?.last, 1578,
                    "candidates that agree give an answer")
        // Two tables of the same size that disagree, and neither is sram.
        Check.isNil(states.frequenciesMHz(operatingStates: 2),
